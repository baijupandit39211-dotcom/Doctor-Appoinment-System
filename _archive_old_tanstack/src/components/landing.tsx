...
import {
  ...
  Linkedin, Twitter, Facebook, Menu, X,
} from "lucide-react";
    ...
    >
      <nav className="mx-auto max-w-7xl px-6 h-16 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
          {links.map((l) => (
            <li key={l.to}>
              <Link to={l.to} className="hover:text-foreground transition-colors" activeProps={{ className: "text-foreground" }}>{l.label}</Link>
            </li>
          ))}
        </ul>
        <div className="hidden md:flex items-center gap-3">
          <Link to="/signin" className="text-sm font-medium px-4 py-2 rounded-full hover:bg-secondary transition">Sign In</Link>
          <Link to="/get-started" className="text-sm font-medium px-5 py-2.5 rounded-full bg-foreground text-background hover:opacity-90 transition shadow-soft">
            Get Started
          </Link>
        </div>
        ...
        {open && (
          ...
          >
            <div className="px-6 py-4 flex flex-col gap-3">
              {links.map((l) => <Link key={l.to} to={l.to} onClick={() => setOpen(false)} className="text-sm py-2">{l.label}</Link>)}
              <Link to="/get-started" onClick={() => setOpen(false)} className="text-sm font-medium py-2.5 rounded-full bg-foreground text-background text-center">Get Started</Link>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="rounded-3xl bg-white overflow-hidden border border-border">
          ...
          <div className="grid grid-cols-12 gap-4 p-5">
            ...
            <div className="col-span-12 md:col-span-10 grid grid-cols-6 gap-4">
              ...
              {/* video widget */}
              <div className="col-span-6 md:col-span-3 rounded-2xl bg-foreground text-background p-4 relative overflow-hidden">
  ...
  return (
    <section className="relative pt-32 pb-24 overflow-hidden bg-mesh">
      ...
      <div className="relative mx-auto max-w-7xl px-6 text-center">
        ...
        >
          <button className="group inline-flex items-center gap-2 rounded-full bg-foreground text-background px-6 py-3 text-sm font-medium shadow-glow hover:scale-[1.02] transition">
            Start Free Trial
            <ArrowRight className="size-4 group-hover:translate-x-0.5 transition" />
          </button>
          <button className="inline-flex items-center gap-2 rounded-full glass px-6 py-3 text-sm font-medium hover:bg-white transition">
            ...
            {tabs.map((t, i) => (
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition ${
                  active === i ? "bg-foreground text-background shadow-soft" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {t}
              </button>
            ...
            return (
              ...
              >
                <div className="relative mx-auto size-24 rounded-3xl bg-hero-gradient grid place-items-center shadow-glow">
                  ...
                  <span className="absolute -top-2 -right-2 size-7 rounded-full bg-foreground text-background text-xs font-bold grid place-items-center">{i+1}</span>
                </div>
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <div className="mt-8 inline-flex items-center gap-3 p-1 glass-strong border border-border rounded-full">
            <button onClick={()=>setAnnual(false)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${!annual?"bg-foreground text-background":"text-muted-foreground"}`}>Monthly</button>
            <button onClick={()=>setAnnual(true)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition ${annual?"bg-foreground text-background":"text-muted-foreground"}`}>Annual <span className="text-teal ml-1">-20%</span></button>
          </div>
        </div>
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-4xl p-8 border transition ${
                  p.pop ? "bg-foreground text-background border-foreground shadow-glow" : "bg-card border-border shadow-soft"
                }`}
              >
                {p.pop && (
                ...
                <p className={`text-sm mt-1 ${p.pop?"text-background/70":"text-muted-foreground"}`}>{p.d}</p>
                <div className="mt-6 flex items-baseline gap-1">
                  {price === null ? (
                    ...
                    <>
                      ...
                      <span className={`text-sm ${p.pop?"text-background/70":"text-muted-foreground"}`}>/mo</span>
                    </>
                  )}
                </div>
                <button className={`mt-6 w-full py-3 rounded-full text-sm font-medium transition ${
                  p.pop ? "bg-background text-foreground hover:opacity-90" : "bg-foreground text-background hover:opacity-90"
                }`}>
        ...
        <div className="relative rounded-4xl overflow-hidden p-12 md:p-20 text-center bg-hero-gradient shadow-glow">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <button className="px-6 py-3 rounded-full bg-white text-foreground text-sm font-semibold hover:scale-[1.02] transition shadow-card">
                Start Free Trial
              </button>
              <button className="px-6 py-3 rounded-full bg-white/15 backdrop-blur border border-white/30 text-white text-sm font-semibold hover:bg-white/25 transition">
                Book Demo
              </button>
            </div>
          </div>
...
export function Footer() {
  const cols: { t: string; l: { label: string; to: string }[] }[] = [
    { t: "Product", l: [{ label: "Features", to: "/features" }, { label: "Pricing", to: "/pricing" }, { label: "Platform", to: "/platform" }] },
    { t: "Resources", l: [{ label: "Resources", to: "/resources" }, { label: "Documentation", to: "/resources" }, { label: "Support", to: "/contact" }] },
    { t: "Company", l: [{ label: "About", to: "/about" }, { label: "Solutions", to: "/solutions" }, { label: "Contact", to: "/contact" }] },
    { t: "Account", l: [{ label: "Sign In", to: "/signin" }, { label: "Get Started", to: "/get-started" }] },
  ];
  return (
    <footer className="border-t border-border bg-secondary/30">
      <div className="mx-auto max-w-7xl px-6 py-16">
          ...
          <div className="col-span-2">
            ...
            <div className="mt-5 flex gap-3">
              {[Linkedin, Twitter, Facebook].map((Icon, i) => (
                <a key={i} href="#" aria-label="social" className="size-9 rounded-full glass-strong border border-border grid place-items-center hover:bg-foreground hover:text-background transition">
                  <Icon className="size-4" />
                </a>
              ))}
            </div>
          ...
          {cols.map((c) => (
            <div key={c.t}>
              ...
              <ul className="mt-4 space-y-2.5">
                {c.l.map((x) => (
                  <li key={x.label}><Link to={x.to} className="text-sm text-muted-foreground hover:text-foreground transition">{x.label}</Link></li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="mt-12 pt-6 border-t border-border flex flex-wrap justify-between items-center gap-3 text-xs text-muted-foreground">
          <p>© {new Date().getFullYear()} DocPulse, Inc. All rights reserved.</p>
